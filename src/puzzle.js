...
const readPuzzle = require('./image-processing/readPuzzle');
const control = require('./control/puzzle');
...
async function main() {
  ...
  while (!mouseMoved(position, robot.getMousePos())) {
    const { result, operators: rawOperators, numbers: rawNumbers } = readPuzzle(position);
